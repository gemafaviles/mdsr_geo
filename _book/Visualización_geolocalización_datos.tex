...
\hypersetup{
  pdftitle={Visualización y geolocalización de datos con R},
...
\chapter{\texorpdfstring{La revolución de los \index{geodatos}}{La revolución de los }}\label{la-revoluciuxf3n-de-los}}
...
\index{Data Scientist} se ha convertido en la \textbf{profesión más sexy del siglo XXI}
...
\textbf{Receptor de posicionamiento Global} (en inglés, \index{Global Positioning System},
...
como \textbf{GIS} por las siglas de su nombre en inglés \index{Geographical Information
...
conocida como \index{Spatial Data Scicene} (SDS).
...
\textbf{¿Qué hace un \index{Sistemas de información geográfico}?}
...
Pero es más, el desarrollo de la \index{**Inteligencia Artificial**} y la \textbf{Inteligencia
...
\textbf{¿Y que es la \index{Geocomputación}?}
...
\textbf{¿Por que \index{R} para datos geográficos?}
...
Los \textbf{datos espaciales}, también conocidos como datos \textbf{\index{geoespaciales}}, son
...
\textbf{Información de \index{vectores}}
...
\hypertarget{raster}{%
\subsection{\texorpdfstring{Datos \index{ráster}}{Datos }}\label{raster}}
...
\hypertarget{CRS}{%
\section{\texorpdfstring{\index{Sistema de Referencia de Coordenadas} (CRS)}{ (CRS)}}\label{CRS}}
...
La \index{estadística espacial} se basa en la suposición de que las unidades
...
\section{\texorpdfstring{Antes de continuar\ldots{} \index{dependencia espacial}}{Antes de continuar\ldots{} }}\label{antes-de-continuar}}
...
De forma natural y de acuerdo a la \index{Ley Tobler} \citep{tobler1970} surge la idea de que
...
\ref{fig:points-depiid} presenta unos \index{datos simulados} donde que muestran una
...
este caso los \index{campos aleatorios} simulados se representan en una rejilla.
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{\index{Datos geoestadísticos}:} Surgen cuando el dominio de estudio es \textbf{continuo
...
\setcounter{enumi}{1}
\item
  \textbf{\index{Datos reticulares}}: Surgen cuando: (i) el dominio bajo estudio \(D\) es
...
\setcounter{enumi}{2}
\item
  \textbf{\index{Procesos de puntos}:} Mientras que en los datos geoestadísticos y
...
carpeta \texttt{data} del proyecto de \index{R-Studio} en el que se esté trabajando.
...
\NormalTok{coords }\OtherTok{\textless{}{-}}\NormalTok{ crimen\_sf }\SpecialCharTok{\%\textgreater{}\%}
