...
airp %>%
  st_drop_geometry() %>%
  bind_cols(df) %>%
  select(Ciudad = AIRP_CITY, longitud, latitud) %>%
  head() %>%
...
1.  **Es un formato multiarchivo:** El formato shapefile se compone de diversos
    ...
    los archivos con extensiones `.shp`, `.shx`, `.dbf` (por ejemplo:
    `datos.shp`, `datos.shx`, `datos.dbf` ). Lo mas habitual es distribuirlos en
    un fichero `.zip` del que se deben extraer todos estos archivos.
...
[mappingGIS](https://mappinggis.com/2012/08/shapefiles-vs-bases-de-datos-espaciales/#Desventajas_de_los_shapefiles),
<http://switchfromshapefile.org/>). Un buen **reemplazo** de este formato **es
el formato GeoPackage** (`.gpkg` ), que no tiene este tipo de limitaciones.
