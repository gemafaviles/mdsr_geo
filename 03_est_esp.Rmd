...
dependencia espacial a la hora de llevar a cabo labores de predicción. 
Por extensión, la estadística espacio-temporal incorpora, además, 
el tiempo y su interacción con
...
La Fig. \@ref(fig:points_dep_iid) muestra unos datos simulados que presentan
una estructura de dependencia espacial (panel izquierdo) frente a unos datos
totalmente aleatorios (panel derecho), en ambos casos disribuidos de 
forma irregular en el espacio.

```{r points_dep_iid, echo=FALSE, message=FALSE, warning=FALSE}
library(geoR)
library(fields)
...
La Fig. \@ref(fig:lattice_dep_iid), al igual que la Fig. \@ref(fig:points_dep_iid)
presenta unos datos simulados donde que presentan
una estructura de dependencia espacial (panel izquierdo) frente a unos datos
 totalmente aleatorios (panel derecho), pero en este caso estos datos
se distribuyen de forma ordenada en el espacio a través de una rejilla regular.

```{r lattice_dep_iid, echo=FALSE, message=FALSE, warning=FALSE}
par(mfrow = c(1, 2))
set.seed(2022)
...
* Los municipios de Castilla-la Mancha están representados por polígonos
con un contorno negro y se rellenan de colores de acuerdo con la variable que 
estamos analizando, el porcentaje de mujeres en los municipios
de Castilla-La Mancha en el año 2019. 

[Lo he multiplicado por cien, creo que faltaba... échale un ojo**]{style="color:red"}
...
* La variable, el porcentaje de mujeres en los municipios
de Castilla-La Mancha en el año 2019, no parece distribuirse de manera independiente
sino todo lo contrario, muestra un patrón espacial. Los municipios del las
...
Antes de dibujar la Fig. (\@ref(fig:mapa_Clm)) tuvimos que leer los datos 
de la librería `mapSpain` que contenía tanto la variable que hemos analiazado
como el formato del mapa. Tras unir variable y mapa con la función `merge()`. 
Al llevar a cabo un resumen del objeto espacial nos encontramos con las siguiente
información:
...
* tiene dimensión XY, lo que indica que cada punto constará de 2 valores de coordenadas.
...
geoestadísticos, datos de patrones de puntos y datos latice (véase Fig. \@ref(fig:hengl_cressie)).


```{r hengl_cressie, echo=FALSE, fig.align='center', fig.cap='Clasificación de datos espaciales propuesta propuesta por @cressie1993. Imangen tomada de [Hengl (2009)](http://spatial-analyst.net/book/system/files/Hengl_2009_GEOSTATe2c1w.pdf) '}
...
1.  **Datos geoestadísticos:** Surgen cuando el dominio de estudio es **continuo
    ...
    concentraciones de un metal pesado en una región. Sin embargo, en la práctica, 
    no es posible una observación exhaustiva del
