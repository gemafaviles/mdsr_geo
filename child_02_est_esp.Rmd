...
de estadística espacial. Veamos un ejemplo simulado de unos datos que muestras dependencia espacial y otros puramente aleatorios.

```{r points_dep_iid, echo=FALSE}
library(geoR)
library(fields)
...
points.geodata(sim1, main = "Dependencia espacial (positiva)", col = tim.colors(),  cex.max = 3)
...
```{r lattice_dep_iid, echo=FALSE}
par(mfrow=c(1,2))
set.seed(2022)
sim2 <- grf(441, grid = "reg", cov.pars = c(1, 0.25))
image(sim2, main = "Dependencia espacial (positiva)", col = tim.colors())
