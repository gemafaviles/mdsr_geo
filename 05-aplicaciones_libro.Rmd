...
## Caso 1: Temperatura mínima en España {#tempmin}
...
::: {#ex-crea .exercise name="Temperaturas mínimas: Creación del proyecto"}
Cree un proyecto para trabajar todo lo referente al caso.
:::

::: {#sol-crea .solution}
Para crear un proyecto siga la secuencia: *File> New Proyect* y a continuación
*New File> R Markdown*
:::
...
::: {#ex4 .exercise name="Descripción de los datos (II)"}
...
::: {#ex6 .exercise name="Representación espacial de la clase sf"}
...
::: {#ex7 .exercise name="Comprobar el CRS de los datos"}
...
::: {#ex8 .exercise name="Cambiar (proyectar) CRS"}
...
::: {#ex9 .exercise name="Mapa con ggplot2"}
...
::: {#ex10 .exercise name="Representar la temperatura mínima en España"}
...
::: {.exercise name="Representar la temperatura mínima con ggplot2"}
...
```{r spatial-plots, fig.cap="Mapa completo con temperatura mínima (8-enero-2021)"}
...
::: {#ex11 .exercise name="Obtención de CRS sugerido para un conjunto de datos"}
...
::: {#ex14 .exercise name="Mapa de interpolación y contorno con ráster"}
Represente los valores interpolados, `tmin_idw`, y añada unas líneas de
contorno. Utilice el paquete **raster** para convertir el objeto interpolado a
pixeles.
:::

```{r, fig.cap="Mapa ráster con líneas de nivel"}
...
::: {#ex15 .exercise name="Mapa de interpolación con ggplot2"}
...
## Caso 2. Distribución de la renta por municipios
...
::: {#ex-crearenta .exercise name="Renta: Creación del proyecto"}
Cree un proyecto para trabajar todo lo referente al caso.
:::

::: {#sol-crearenta .solution}
...
software de tratamiento (en nuestro caso, **R**) y analizarlos para conocer el
tipo de información que contiene.

::: {#ex16 .exercise name="Importación y análisis de los datos objeto de estudio"}
...
**Cuando crucemos datos espaciales con datos no espaciales en** R**, es
...
::: {#ex21 .exercise name="Adecuación de los datos a la visualización"}
...
que en **R** se encuentran implementados en el paquete **classInt**
[@R-classint]. A continuación se van a plantear diversos métodos de
...
## Caso 3. Delitos cometidos en Valencia en el año 2010
...
-   **GEMA, esto está a medio terminar?** describir los ficheros. Hay que
    generar uno que sólo tenga 2010 y los datos que utilizamos. lo llamamos
    igual y vale todo

::: {#ex24 .exercise name="Patrones de puntos: Creación del proyecto"}
Cree un proyecto para trabajar todo lo referente al caso.
:::
...
::: {#ex25 .exercise name="Importación de los datos de crímenes"}
Importe los datos `crime-data-Valencia.csv` y guárdelos en un objeto llamado
`crimen`.
:::
...
**readr** [@R-readr] para importar los datos:
...
::: {#ex26 .exercise name="Descripción de los datos: Crímenes en Valencia"}
¿Qué información contiene el archivo importado?
:::

::: {#sol26 .solution}
Use la función `summary()` en el objeto `crimen` para obtener información de los
datos.
:::
...
::: {#ex27 .exercise name="Identificación del CRS de los datos proporcionados"}
Determine el CRS en el que se encuentran las coordenadas del conjunto de datos
...
::: {#ex28 .exercise name="Proyección y representación de localizaciones"}
...
::: {#ex29 .exercise name="Filtrado de datos"}
Seleccione los crímenes ocurridos en el año 2010 y almacénelos en un objeto
llamado `crimen_2010_sf` Opcionalmente, represente esos datos en un mapa. Puede
...
::: {#ex30 .exercise name="Análisis de patrones con spatstat"}
Analice la existencia de patrones en los crímenes registrados en el año 2010.
:::
...
Valencia, que podemos extraer del paquete **mapSpain**.

```{r objeto-ppp, message=FALSE, warning=FALSE, fig.cap="Patrones de puntos en los crímenes de Valencia (2010)"}
...
::: {#ex31 .exercise name="Información de objetos ppp"}
¿Qué información contiene nuestro objeto `mydata_ppp`?
:::
...
::: {#ex32 .exercise name="Cálculo de la densidad de los delitos en Valencia en 2010 mediante cuadrantes"}
Cree unos cuadrantes sobre el objeto `mydata_ppp` usando la función
`quadratcount`.
:::

**GEMA: ESte mapa se puede poner un poco más mono?? Yo ya lo he arregado un
poco.** **DIEGO:Yo no se hacerlo. Es lo malo de estas librerias gstat, geoR,
...
::: {#ex33 .exercise name="Estimación de la densidad de los delitos en Valencia en 2010 mediante de patrones de puntos"}
Analice la densidad de crímenes en Valencia usando la función `density()`.
:::
...
::: {.exercise name="Representación alternativa con ggplot2"}
Recree el mapa de la Fig. \@ref(fig:intensidad-ppp) con **ggplot2**.
:::
...
En este caso, al convertir del formato de `spatstat` a `sf`, se ha perdido la
definición del CRS:
...
Una vez que se dispone del objeto espacial `pols` completo, es posible usar
**ggplot2** de la manera habitual

```{r, fig.cap="Estimación kernel con ggplot2"}
...
ggplot() +
  ...
  geom_sf(
    data = crimen_2010_sf, col = "grey90",
    size = 0.3,
    alpha = 0.1
  ) +
  # Estética del mapa
