...
valencia <- esp_get_munic(munic = "^Valencia$") %>%
  st_transform(25830) ## esto está bien??
...
# cojo, el 2010
mydata_crime <- crime_todos %>% filter(year==2010)
...
mydata_crime_sf <- mydata_crime %>%
  st_as_sf(coords = c("crime_lon", "crime_lat"),
  crs = st_crs(25830)
)
```
...
mydata_crime_sfc <- st_as_sfc(mydata_crime)
mydata_crime_sf <- st_as_sf(mydata_crime_sfc)
...
mydata_points <- SpatialPointsDataFrame(
  coords = data.frame(x=as.numeric(mydata_crime$crime_lon),
...
valencia_bbox <- bbox(cbind(mydata_crime$crime_lon, mydata_crime$crime_lat))
valencia_gg <- get_map(c(valencia_bbox), maptype='watercolor', source='stamen')
ggmap(valencia_gg)
...
st_crs(mydata_points)==st_crs(valencia["cmun"])
